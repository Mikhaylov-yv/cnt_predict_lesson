#!/usr/bin/env python3
"""
Контейнерный предиктор на основе трансформеров с метриками для агрегатов
"""
...
def calculate_aggregate_metrics(predictions, true_aggregates, target_col='Москва_20'):
    """Вычисление метрик для агрегатов"""
    
    # Подсчитываем предсказанные свободные контейнеры в Москве 20
    predicted_counts = []
    true_counts = []
    
    for day in range(len(predictions)):
        # Подсчитываем количество свободных контейнеров в Москве 20
        day_predictions = predictions[day]
        
        # Считаем контейнеры со state='free' и location='Москва' и cnt_type=20
        free_moscow_20_count = 0
        for pred in day_predictions:
            if (pred['predicted_state'] == 'free' and 
                pred['predicted_location'] == 'Москва' and 
                pred['cnt_type'] == 20):
                free_moscow_20_count += 1
        
        predicted_counts.append(free_moscow_20_count)
        
        # Берем истинное значение из агрегатов
        if day < len(true_aggregates):
            true_counts.append(true_aggregates.iloc[day][target_col])
        else:
            true_counts.append(0)
    
    # Вычисляем метрики
    mse = mean_squared_error(true_counts, predicted_counts)
    mape = mean_absolute_percentage_error(true_counts, predicted_counts) * 100
...
def predict_and_evaluate(model, test_loader, test_data, state_encoder, location_encoder, 
                        true_aggregates, target_col='Москва_20'):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            predicted_states = torch.argmax(state_probs, dim=-1).cpu().numpy()
            predicted_locations = torch.argmax(location_probs, dim=-1).cpu().numpy()
            
            # Декодируем
            for i in range(predicted_states.shape[0]):
                for j in range(predicted_states.shape[1]):
                    state = state_encoder.inverse_transform([predicted_states[i, j]])[0]
                    location = location_encoder.inverse_transform([predicted_locations[i, j]])[0]
                    
                    predictions.append({
                        'predicted_state': state,
                        'predicted_location': location,
                        'cnt_type': 20  # Для простоты считаем все 20-футовыми
                    })
    
    # Вычисляем метрики
    metrics = calculate_aggregate_metrics(predictions, true_aggregates, target_col)
    
    return predictions, metrics

def main():
    """Основная функция"""
    print("Запуск контейнерного предиктора с метриками...")
    ...
    # 6. Предсказание и оценка метрик
    print("Предсказание и оценка метрик...")
    
    # Загружаем истинные агрегаты для сравнения
    ...
    predictions, metrics = predict_and_evaluate(
        model, test_loader, test_data, 
        state_encoder, location_encoder, 
        true_aggregates, target_col='Москва_20'
    )
    
    # Выводим метрики
    print("\n" + "="*50)
    print("МЕТРИКИ ДЛЯ КОЛИЧЕСТВА СВОБОДНЫХ КОНТЕЙНЕРОВ МОСКВА_20")
    print("="*50)
    ...
    plt.subplot(2, 2, 2)
    plt.plot(metrics['true'], label='True', marker='o')
    plt.plot(metrics['predicted'], label='Predicted', marker='s')
    plt.title('Предсказания vs Истинные значения')
    plt.xlabel('День')
    plt.ylabel('Количество контейнеров')
    plt.legend()
    
    plt.subplot(2, 2, 3)
    plt.scatter(metrics['true'], metrics['predicted'], alpha=0.6)
    plt.plot([min(metrics['true']), max(metrics['true'])], 
             [min(metrics['true']), max(metrics['true'])], 'r--', lw=2)
    plt.title('Scatter Plot: True vs Predicted')
    plt.xlabel('True Values')
    plt.ylabel('Predicted Values')
    
    plt.subplot(2, 2, 4)
    residuals = np.array(metrics['predicted']) - np.array(metrics['true'])
    ...
    torch.save({
        ...
        'global_feature_dim': global_feature_dim,
        'metrics': metrics
    }, 'container_transformer_model_with_metrics.pth')
    
    print("Модель сохранена в container_transformer_model_with_metrics.pth")
