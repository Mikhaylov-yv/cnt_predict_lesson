#!/usr/bin/env python3
"""
Быстрый анализ метрик для агрегатов контейнеров
"""
...
def train_and_evaluate_model(df, target_col='Москва_20'):
    ...
    plt.scatter(y_test, pct_errors, alpha=0.6)
    plt.xlabel('True Values')
    ...
    plt.subplot(3, 4, 12)
    metrics_names = ['MSE', 'MAPE', 'R²']
    metrics_values = [mse, mape/100, r2]  # Нормализуем MAPE
    ...
    for bar, value in zip(bars, [mse, mape, r2]):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.01, 
                f'{value:.3f}', ha='center', va='bottom')
    
    plt.tight_layout()
    plt.savefig(f'metrics_analysis_{target_col}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    """Основная функция"""
    print("Запуск быстрого анализа метрик...")
